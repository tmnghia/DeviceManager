...
int main(int argc , char *argv[])
{
	...
	while(1) {
		...
		if (FD_ISSET(sfd, &readfds)) {
			newsfd = accept(sfd, (struct sockaddr*)&svaddr, (socklen_t*)&addrlen);
			memset(buffer, 0, 256);
			...
			if (strcmp(buffer, "show") == 0) {
				...
				for (i=0; i<MAX; i++) {
					if (client[i] != 0) {
						Send_int(newsfd, i);
						Send_char(newsfd, cli[i].user);
						Send_char(newsfd, cli[i].cpu);
						Send_char(newsfd, cli[i].memory);
						Send_char(newsfd, cli[i].ip);
						Send_int(newsfd, cli[i].port);
					}
				}
				close(newsfd);
			}
